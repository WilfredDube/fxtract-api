...
type SGMailService struct {
	configs *configuration.ServiceConfig
}

// NewSGMailService returns a new instance of SGMailService
func NewSGMailService(configs *configuration.ServiceConfig) *SGMailService {
...
func (ms *SGMailService) CreateMail(mailReq *Mail) []byte {
	...
	from := mail.NewEmail("Fxtract", mailReq.from)
	m.SetFrom(from)
	m.Subject = "Welcome to Fxtract! Confirm your email"
